...
Telegram-бот (Max) и веб-приложение для музеев Ставропольского края: каталог, геолокация, поиск, распознавание экспонатов по фото (EfficientNetB0).
...
```
sk-museum-bot/
├── main.go                  # Telegram-бот (Go) — точка входа
