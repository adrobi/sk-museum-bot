...
После запуска база данных заполнится 47 музеями из файла init.sql. Чтобы заработало распознавание экспонатов нужно зайти в веб-приложение (или mini-app) и зайти через главного админа, тогда станет возможно обучить модель для каждого музея (до 20 фото на экспонат, разметка данных не требуется).
